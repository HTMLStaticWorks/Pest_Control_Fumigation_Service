...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // THREE.JS 3D HERO SHIELD & MOLECULES SCENE
  initThreeJSScene();
...
function initThreeJSScene() {
  ...
  const dirLight2 = new THREE.DirectionalLight(0x84CC16, 1.8);
  dirLight2.position.set(-5, -5, 2);
  scene.add(dirLight2);

  // Create 3D Shield Mesh Group
  ...
  shape.moveTo(0, 2);
  shape.bezierCurveTo(1.2, 1.8, 1.8, 1.2, 1.8, 0.4);
  shape.bezierCurveTo(1.8, -1.2, 0.8, -2.0, 0, -2.8);
  shape.bezierCurveTo(-0.8, -2.0, -1.8, -1.2, -1.8, 0.4);
  shape.bezierCurveTo(-1.8, 1.2, -1.2, 1.8, 0, 2);

  const extrudeSettings = { depth: 0.35, bevelEnabled: true, bevelSegments: 6, steps: 2, bevelSize: 0.1, bevelThickness: 0.1 };
  const shieldGeometry = new THREE.ExtrudeGeometry(shape, extrudeSettings);
  ...
  // Outer Glowing Ring
  const ringGeom = new THREE.TorusGeometry(2.6, 0.04, 16, 100);
  const ringMat = new THREE.MeshStandardMaterial({ color: 0x84CC16, emissive: 0x65A30D, emissiveIntensity: 0.5 });
  const ringMesh = new THREE.Mesh(ringGeom, ringMat);
  shieldGroup.add(ringMesh);

  // Floating Molecules Orbiting Shield
  const molecules = [];
  const sphereGeom = new THREE.SphereGeometry(0.18, 16, 16);
  const molMat = new THREE.MeshStandardMaterial({ color: 0x14B8A6, roughness: 0.1 });

  for (let i = 0; i < 8; i++) {
    const mol = new THREE.Mesh(sphereGeom, molMat);
    const angle = (i / 8) * Math.PI * 2;
    const radius = 3.2;
    mol.position.set(Math.cos(angle) * radius, Math.sin(angle) * radius, (Math.random() - 0.5) * 1.5);
    shieldGroup.add(mol);
    molecules.push({ mesh: mol, angle: angle, speed: 0.01 + Math.random() * 0.01 });
  }
  ...
  // Mouse Parallax Effect (passive & paused when off-screen)
  ...
  // IntersectionObserver to pause 3D rendering when scrolled away
  ...
  function animate() {
    ...
    shieldGroup.rotation.y = Math.sin(elapsedTime * 0.5) * 0.2 + mouseX * 0.8;
    shieldGroup.rotation.x = Math.cos(elapsedTime * 0.5) * 0.15 - mouseY * 0.8;
    shieldGroup.position.y = Math.sin(elapsedTime * 1.2) * 0.15;

    ringMesh.rotation.z += 0.005;

    // Rotate Orbiting Molecules
    molecules.forEach(m => {
      m.angle += m.speed;
      m.mesh.position.x = Math.cos(m.angle) * 3.2;
      m.mesh.position.y = Math.sin(m.angle) * 3.2;
    });
  ...
  // Resize Handler (Passive & Throttled)
